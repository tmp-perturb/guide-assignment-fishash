#!/usr/bin/env Rscript
#
# run_fishash.R — fishash runner for assignment benchmark
#
# Reads a merged MEX trio, runs fishash(), unpacks assignments to
# standard CSV format (cell, gRNA, UMI_counts), and writes
# monitoring JSON.
#
# Usage:
#   Rscript run_fishash.R \
#       --mex-dir 04_extraction_mex/cellranger \
#       --output  09_fishash/cellranger \
#       --padj-cutoff 0.05 \
#       --padj-method GS \
#       --min-count 2 \
#       --refit 10
#
# Note: .libPaths must include /home/yunzliu/R/library

# ── Lib path ──────────────────────────────────────────────────────────
.libPaths(c("/home/yunzliu/R/library", .libPaths()))

# ── Parse args ────────────────────────────────────────────────────────
library(optparse)

option_list <- list(
    make_option("--mex-dir", type="character", help="MEX trio directory"),
...
if (is.null(args$`mex-dir`) || is.null(args$output)) {
    print_help(parser)
    quit(status=1)
}

mex_dir  <- sub("/$", "", args$`mex-dir`)
out_dir  <- sub("/$", "", args$output)
tool     <- basename(out_dir)
...
# Read gzipped Matrix Market (MEX: rows=cells, cols=guides)
# fishash expects rows=guides, cols=cells → transpose
...
# Uses sparse matrix indexing (O(nnz)).  Extracts log_pval and
# odds_ratio_regularized alongside UMI_counts for downstream ranking.
...
# Dense all-cell scores for per-cell quantile ranking
logpval_full  <- assay(res, "log_pval")
or_full       <- assay(res, "odds_ratio_regularized")

# Assigned matrix as sparse TsparseMatrix
...
out_csv <- file.path(out_dir, "assignments.csv")
...
gpc <- table(df$cell)  # guides per cell
